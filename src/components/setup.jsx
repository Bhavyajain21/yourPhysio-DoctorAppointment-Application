...
class SetUp extends React.Component {
    ...
    //     {
    //         let ok = localStorage.getItem("starting");
    //         const starting = ok.split(',');
    //         console.log(starting);
    //         this.setState({starting});
    //     }
    //     if(localStorage.getItem("ending")!=null)
    //     {
    //         const ok = localStorage.getItem("ending");
    //         const ending = ok.split(',');
    //         this.setState({ending});
    //     }
    //     const starting = this.state.starting;
    //     const ending = this.state.ending;
    //     const src1 = this.state.src1;
    //     const dest1 = this.state.dest1;
    //     for(let i=0;i<7;i++)
    //     {
    //         let s = starting[i];
    //         let ok = parseInt(s.slice(0,2));
    //         src1[i] = ok;

    //         let e = ending[i];
    //         let k = parseInt(e.slice(0,2));
    //         dest1[i] = k;
    //     }
    //     this.setState({src1,dest1});
    // }
    render() {
        let today = new Date();
                ...
                <div className="inner">
                    {   
                        mainArr.map(e => <>
                                            ...
                                            />
                                            <button className="saveBtn" onClick={()=>this.addInTotal(e)}>Save</button>
                                            </div>
                                            {idxPressed==e?<SubInput arr = {this.state.arrObj[e]}/>:<></>}
                                        </>)
                    }
                    <div className="cover">
                        <button className="submitBtn"  onClick={this.displayHrs}>Submit</button>
                    </div>
                    {this.state.x && <h1 className="totalHrs">Total Working Hours : {this.state.total}</h1>}
                </div>
            </div>
        );
    }
